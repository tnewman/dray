...
impl TryFrom<&Bytes> for Path {
    type Error = Error;

    fn try_from(path_bytes: &Bytes) -> Result<Self, Self::Error> {
...
mod test {
    ...
    fn test_parse_path_with_invalid_id() {
        ...
        assert_eq!(Path::try_from(&path_bytes.freeze()), Err(Error::BadMessage));
    }
